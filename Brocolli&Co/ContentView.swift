...
struct ContentView: View {
    ...
    //user default get the value stored
    @State var isLoggedin: Bool =  UserDefaults.standard.bool(forKey: "myLogin")
    //email check backend validation: usedemail@blinq.app
    //first view
    var body: some View {
        //load different view if user asked for invite
        if(isLoggedin)
        {
            CancelInviteView()
        } else {
            NavigationView {
                ...
                .navigationBarTitle("Brocolli & Co.")
...
struct InviteView: View {
    ...
    var body: some View{
        ZStack {
            ...
            .navigationBarTitle("Invite Details")
            .padding()
...
struct CongratulationsView: View {
    ...
    @State private var isLogggedin = true
    
    var body: some View {
        VStack{
            ZStack{
                //confetti
                Text("🥦").font(.system(size: 50)).onAppear(){counter7 += 1}
...
struct CancelInviteView: View {
    ...
    @State var isNavBarHidden = false
    @State private var isLogggedin = false
...
struct CancelView: View {
    ...
    var body: some View {
        VStack{
            ZStack{
                Text("💩").font(.system(size: 50)).onAppear(){counter6 += 1}
